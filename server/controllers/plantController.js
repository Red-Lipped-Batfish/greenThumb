...
async function getPlantsInfo() {
  ...
  for (let pageNumber = 1; pageNumber < 500; pageNumber++) {
    ...
    if (response.ok) {
      ...
      console.log('responseObj =', json[0].scientific_name);
  ...
  for (let i = 0; i < resultArray.length; i++) {
    ...
    if (response.ok) {
      ...
      if (species.growth.precipitation_maximum.inches) {
        ...
        resultObj.precipitation_maximum =
          species.growth.precipitation_maximum.inches;
      }
      if (species.growth.precipitation_minimum.inches) {
        console.log(species.growth.precipitation_minimum.inches);
        resultObj.precipitation_minimum =
          species.growth.precipitation_minimum.inches;
      }
      ...
      if (species.seed.commercial_availability) {
        ...
        resultObj.commercial_availability =
          species.seed.commercial_availability;
      }
      ...
      if (
        species.specifications.length &&
        species.specifications.growth_habit
      ) {
        console.log(species.specifications.growth_habit);
        resultObj.species.specifications.growth_habit =
          species.specifications.growth_habit;
      }
      if (
        species.specifications.length &&
        species.specifications.growth_period
      ) {
  ...
  plants.forEach(currentPlant => Plant.create(currentPlant));
}

// get data and add to database
plantController.getData = (req, res) => {
  const plantsInfo = getPlantsInfo(res);
